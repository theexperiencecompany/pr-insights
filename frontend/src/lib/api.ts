...
export const getContributors = (params: { repo?: string; q?: string; page?: number; per_page?: number; from?: string; to?: string } = {}): Promise<ContributorsData> => fetch(`/api/contributors${qs(params)}`).then(json<ContributorsData>)
