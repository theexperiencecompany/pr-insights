...
export const getContributors = (params: { repo?: string; q?: string; page?: number; per_page?: number; bot?: string; from?: string; to?: string } = {}): Promise<ContributorsData> => fetch(`/api/contributors${qs(params)}`).then(json<ContributorsData>)
